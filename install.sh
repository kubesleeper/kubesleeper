...
then
  echo "namespace '$NAMESPACE' exists, delete all resources of it"
  kubectl delete all --all -n $NAMESPACE >/dev/null
else
    echo "namespace '$NAMESPACE' do not exists, creating it"
    if kubectl create namespace $NAMESPACE >/dev/null
    then
        ok "'$NAMESPACE' namespace created"
    else err "failed to create '$NAMESPACE' namespace"
    fi
fi
