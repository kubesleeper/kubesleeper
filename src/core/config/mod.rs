...
///
/// If a path is provided, it will attempt to load the configuration from that file.
/// If no path is provided, it will look for a default configuration file named `kubesleeper.yaml`.
/// If the default file is not found, it returns the default configuration.
pub fn parse(path: Option<PathBuf>) -> Result<Config, ConfigError> {
    let path: Option<PathBuf> = match path {
        ...
        // No config file specified looking for the default path
        None => {
